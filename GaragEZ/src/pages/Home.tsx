...
const Home: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (!container) {
      console.error("snapContainer not found");
      return;
    }
    ...
    const handleUserActivity = () => {
      ...
      idleTimeout = setTimeout(autoScroll, 5000);
    };
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      const currentIndex = Math.round(currentScrollTop / containerHeight); // Math.floor 대신 Math.round로 정확도 개선
    ...
    const autoScroll = () => {
      ...
      smoothScrollTo(container, nextIndex * containerHeight, 1300, setHideImage);
      autoScrollTimeout = setTimeout(autoScroll, delay);
    };
    ...
    autoScrollTimeout = setTimeout(autoScroll, 3500);
        ...
        {[
          ...
          <section key={index} className={styles.section}>
            ...
            <div className={styles.buttonContainer}>
              <Link to={item.link} className={styles.pageButton}>
                {item.text}
              </Link>
            </div>
          </section>
